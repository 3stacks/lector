...
import type { Book } from '@/lib/data-layer';
...
export default function BookCard({ book }: BookCardProps) {
  const progressPercent = Math.round(book.progress.percentComplete);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {book.author && (
          <p className="mt-1 truncate text-sm text-zinc-500 dark:text-zinc-400">{book.author}</p>
        )}
        <div className="mt-auto pt-3">
          <span className="inline-flex items-center gap-1 text-xs text-zinc-400 dark:text-zinc-500">
            ...
            {book.lastReadAt ? formatLastRead(book.lastReadAt) : 'Not started'}
          </span>
        </div>
      </div>
    </Link>
